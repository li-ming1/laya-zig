...
> 前向也与独立参考实现对齐过，性能也认真优化过一轮（见「实测」：笔记本 CPU 上
> 一步决策 ~200 ms），但请把它当作一个**半成品**：边角粗糙、功能不全，
> 也还没有在真实场景里用过。
>
> 欢迎提 issue、指正和 PR。
...
本项目是**同一套权重的独立 Zig 重实现**，目标是：单二进制、无依赖、可离线、读完 614 MB
权重后约半秒内就能开始答题。
...
线程数自动取 12。每列都是多次重复的中位数，而区间的宽度就是机器本身：持续加载几
分钟之后的稳定频率比头几次慢 ~20%，所以前后是同一状态下测的 —— 先凉机测一对，
再热机测一对。「优化前」是上一个 commit，在同一台机器上同样编译。

| 项目 | 优化前 | 现在 | 倍率 |
|---|---|---|---|
| 内置 demo（2 个问题，63 + 64 token） | 844–1017 ms | **124–143 ms** | ~7× |
| 蛇探针（一步决策，175–180 token） | 1909–2160 ms | **202–224 ms** | ~9.5× |
| 权重加载（614 MB f16 → f32） | 593–708 ms | **376–406 ms** | ~1.7× |
| 分词器加载（256k 词表 / 580,604 merges / 249 added） | 136–151 ms | 137–153 ms | — |
| 常驻内存 | 1177 MB | 1177 MB | — |

蛇那一行决定网页界面像不像「实时」：~200 ms 一步就是每秒 5 次决策。中位数取自一局
6 步；最快的一步从 901 ms 变成 174 ms，旧代码越打越慢（每次并行都新建线程 +
温度漂移），新代码基本持平。

两个数量级的提速没有拿精度去换：两列打印的选择与概率逐位相同。`--selftest` 里
GEMM 相对 f64 的最大误差 7.98e-6，`tools/refcheck.py` 用纯 Python 独立复算 22 层
加两个头，逐层偏差 2.6e-5。

`--profile` 给出的耗时分布：matmul 84%、attention 6%、norm 3%、residual 3%、
RoPE 1%、GLU 1%、QKV 拆分 1%。matmul 每次前向流式读 441 MB f32 权重，约
250 GFLOP/s；它下面的管线已经只剩几毫秒，所以剩下的空间要么在 GEMM 核本身，要么
把权重以 f16 常驻、在核内转换 —— 那样能同时减半内存流量和 1177 MB 常驻，但这块
芯片未必划得来。

线程数默认 `min(核数, 12)`。同一台机器上 demo forward：4 线程 253 ms、8 线程
170 ms、12 线程 153 ms、**16 线程 640 ms**。超过每物理核一条线程之后并不是更并行，
而是 SMT 抢占，屏障要等最慢的那一个。
